...
export const ListGenerate = ({ulStyle, list, classStyle, funcItems, shadow=false, cards=false, flag=false, gamePlay=false, noMargin=false, children=null, func, childrenStyle, childrenText}: IListGenerate) => { 
  const len = list.length 
  //const arr = Array.from(' '.repeat(3)) 
  const arr = Array.from({length:3})
  return (
        <ul className={ulStyle}>{list.map((card:TypeDeck, index:number) => 
          <li key={Math.random().toString(36).substring(2, 15)}
              onClick={funcItems ? () => funcItems(card) : () => console.log()} 
              className={classStyle}  style={cards ? 
                { transform: !gamePlay ? 
                  !noMargin ? `translate(${-index/2}px, ${-index/2}px)` 
                  : `translateX(${index*(600/len)}%)`
              :
              `translate(${index*22}%, ${-index*2}%) 
                rotate(${len>1 ? index<(Math.floor(len/2)) 
                  ? -len+index 
                  : len+index : 0}deg)` }  
              :
              {}}>
      ...
      {children && <button className={childrenStyle} onClick={() => {func && func()}}>
        {childrenText}
      </button>}
      </ul>
      ) 
